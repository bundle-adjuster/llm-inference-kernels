...
| Phase 0 end-to-end (Llama 3.1 8B Instruct, batch 16, prompt 512 / gen 512) | greedy decode, EOS suppressed | HF `generate()` 23.10 s · vLLM 0.6.6 11.65 s | n/a (vendor-baseline phase) | vLLM 1.98× HF `generate()` | ⚠ Phase 7: that 1.98× is transformers' `repeat_kv`, not kernels. One line (`enable_gqa=True`) takes HF to 533 tok/s / 1.55×, cutting vLLM's lead to 1.31×. Phase 8's v6 kernel edges that fair baseline (538.6 tok/s). ✅ **Phase 9 pulled that lever:** W4A16 split-K GEMM (1.66–2.0× over cuBLAS) + cat-free v6 decode + fused prefill dequant reach **756 tok/s — 1.08× vLLM-fp16** (decode-only 1.20×). Caveat: 4-bit vs fp16 (a throughput/accuracy trade — W4A16 costs MMLU 62.4% vs 68.3%; fp16-vs-fp16 we're ~0.85×). Like-for-like *kernels* match/beat vLLM's. [`docs/07`](docs/07-w4a16-e2e-journey.md) |
